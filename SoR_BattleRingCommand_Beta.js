...
/*:ja
* @plugindesc ＜戦闘用アクターリングコマンド Type:β＞
...
* @desc リングの土台画像を置くx座標(default: 300)
* @default 300
* @type number
* @param Ringbase_Y-coordinate
* @desc リングの土台画像を置くy座標(default: 360)
* @default 360
* @type number
*
* @param Ringover_X-coordinate
* @desc リングの上層に重ね置きする画像を置くx座標(default: -15)
* @default -15
* @type number
* @param Ringover_Y-coordinate
* @desc リングの上層に重ね置きする画像を置くy座標(default: -15)
* @default -15
* @type number
*
* @param CommandPadd_X-coordinate
* @desc コマンドアイコンの配置開始位置のx方向の補正量(default: 114)
* @default 114
* @type number
* @param CommandPadd_Y-coordinate
* @desc コマンドアイコンの配置開始位置のy方向の補正量(default: 22)
* @default 22
* @type number
*
* @param CommandName_X-coordinate
* @desc 選択中のコマンド名を表示する位置のx座標(default: 108)
* @default 108
* @type number
* @param CommandName_Y-coordinate
* @desc 選択中のコマンド名を表示する位置のy座標(default: 45)
* @default 45
* @type number
...
*
* @param Ringbase_X-coordinate
* @desc x-coordinate (at upper right corner) of Base Ring sprite (default: 300)
* @default 300
* @type number
* @param Ringbase_Y-coordinate
* @desc y-coordinate (at upper right corner) of Base Ring sprite (default: 360)
* @default 360
* @type number
*
* @param --------------------------------
...
* @param Ringover_X-coordinate
* @desc x-coordinate of the upper layered ring sprite (default: -15)
* @default -15
* @type number
* @param Ringover_Y-coordinate
* @desc y-coordinate of the upper layered ring sprite (default: -15)
* @default -15
* @type number
*
* @param CommandPadd_X-coordinate
* @desc Padding for x to align command icon on the ring (default: 114)
* @default 114
* @type number
* @param CommandPadd_Y-coordinate
* @desc Padding for y to align command icon on the ring (default: 22)
* @default 22
* @type number
*
* @param CommandName_X-coordinate
* @desc x-coordinate to draw command name for current command index (default: 108)
* @default 108
* @type number
* @param CommandName_Y-coordinate
* @desc y-coordinate to draw command name for current command index(default: 45)
* @default 45
* @type number
* @param CommandName_Width
...
var Param = PluginManager.parameters('SoR_BattleRingCommand_Beta');
var Ringbase_X_coord = Number(Param['Ringbase_X-coordinate'] || 300);
var Ringbase_Y_coord = Number(Param['Ringbase_Y-coordinate'] || 360);
var Ringover_X_coord = Number(Param['Ringover_X-coordinate'] || -15);
var Ringover_Y_coord = Number(Param['Ringover_Y-coordinate'] || -15);
var CommandPadd_X_coord = Number(Param['CommandPadd_X-coordinate'] || 114);
var CommandPadd_Y_coord = Number(Param['CommandPadd_Y-coordinate'] || 22);
var CommandName_X_coord = Number(Param['CommandName_X-coordinate'] || 108);
var CommandName_Y_coord = Number(Param['CommandName_Y-coordinate'] || 45);
...
Window_ActorCommand.prototype.processCursorMove = function() {	
	...
	if (this.isOpenAndActive()){
		if (Input.isRepeated('left')) Inputting = 1; 
		if (Input.isRepeated('right')) Inputting = -1;
	}	
	if(Inputting!=0){
...
SoR_BattleRingCommand.prototype.updateCommand = function(ActCom){
	...
	for(var i=0; i<this.maxicons; i++){
		...
		this.command_icons[i].x = this.radius * Math.sin((-rotate_main)*idx +rotate_com) +this.x+CommandPadd_X_coord;
		this.command_icons[i].y = 0.25*this.radius * Math.cos((-rotate_main)*idx +rotate_com) +this.y+CommandPadd_Y_coord;
		this.command_icons[i].visible = true;
